...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/11 10:48:36 by anbaya            #+#    #+#             */
/*   Updated: 2025/03/11 10:50:27 by anbaya           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute(char *env, char *cd)
{
	...
	if (ft_strchr(cmd[0], '/') || !env || !*env)
		return (cut_for_norm(paths, cmd));
	while (paths && paths[i])
	{
		full_cmd = ft_strsjoin((char *[]){paths[i], "/", cmd[0], NULL});
		if (access(full_cmd, F_OK) == 0)
			return (cut_for_norm2(paths, cmd, full_cmd));
		free(full_cmd);
		i++;
	}
	write(2, "Error:\ncommand not found\n", 25);
	return (free_arr(paths), free_arr(cmd), 1);
}
...
{
	if (ac != 5 || !av[1] || !av[1][0] || !av[2] || !av[2][0] || !av[3]
		|| !av[3][0] || !av[4] || !av[4][0])
	{
		write(2, "Error:\ninvalid input\n", 21);
		return (1);
